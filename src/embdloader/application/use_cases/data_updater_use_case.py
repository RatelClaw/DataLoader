...
class DataUpdaterUseCase:
    ...
    async def execute(
        ...
        if embed_type not in ['combined', 'separated']:
            raise DataValidationError(f"Invalid embed_type: {embed_type}. Must be 'combined' or 'separated'.")

        df_new = self.storage_loader.load_csv(s3_uri)
        embed_columns_names = new_embed_columns_names or await self.repo.get_embed_columns_names(table_name)
        ...
        if embed_type == 'combined':
            ...
            # New and updated
            df_to_insert = df_new[~df_new.set_index(pk_columns).index.isin(df_existing.set_index(pk_columns).index)]
            df_common_new = df_new[df_new.set_index(pk_columns).index.isin(df_existing.set_index(pk_columns).index)]
            df_common_existing = df_existing[df_existing.set_index(pk_columns).index.isin(df_common_new.set_index(pk_columns).index)]

            # Find changed
            changed_mask = (df_common_new[data_columns + ['embed_columns_value']] != df_common_existing[data_columns + ['embed_columns_value']]).any(axis=1)
            df_changed = df_common_new[changed_mask].copy()

            # Always recompute embeddings for new and changed (or all if new_embed)
            texts_to_embed = pd.concat([df_to_insert['embed_columns_value'], df_changed['embed_columns_value']])
            if new_embed_columns_names:  # Recompute all common if new config
                texts_to_embed = pd.concat([texts_to_embed, df_common_new[~changed_mask]['embed_columns_value']])
                df_unchanged = df_common_new[~changed_mask].copy()
                df_changed = pd.concat([df_changed, df_unchanged])

            if not texts_to_embed.empty:
                embeddings = self.embedding_service.get_embeddings(texts_to_embed.tolist())
            ...
            for col in embed_columns_names:
                ...
                if enc_col not in schema.columns:
                    await self.repo.add_column(table_name, enc_col, f"vector({DEFAULT_DIMENTION})")
            ...
            # Find changed
            changed_mask = (df_common_new[data_columns] != df_common_existing[data_columns]).any(axis=1)
            df_changed = df_common_new[changed_mask].copy()
    ...
    def _get_removed_pks(self, df_existing: pd.DataFrame, df_new: pd.DataFrame, pk_columns: List[str]) -> List[tuple]:
    ...
    async def _validate_schema(self, df: pd.DataFrame, table_name: str, embed_columns_names: List[str]):
        # Same as in loader
        schema = await self.repo.get_table_schema(table_name)
        data_columns = await self.repo.get_data_columns(table_name)
        if set(df.columns) != set(data_columns):
            raise DataValidationError(f"CSV columns {list(df.columns)} do not match table data columns {data_columns}")
        if not set(embed_columns_names).issubset(data_columns):
            raise DataValidationError(f"embed_columns_names not in data columns {data_columns}")
        ...
        for col in data_columns:
            ...
            if pd_to_pg.get(pd_type) != expected_pg:
                raise DataValidationError(f"Type mismatch for {col}: {pd_type} vs {expected_pg}")
