...
class DataLoaderUseCase:
    ...
    async def execute(
        ...
        if not all(col in df.columns for col in pk_columns):
            raise DataValidationError(f"Primary key columns {pk_columns} not in DataFrame columns {list(df.columns)}")
        if not all(col in df.columns for col in embed_columns_names):
            raise DataValidationError(f"Embed columns {embed_columns_names} not in DataFrame columns {list(df.columns)}")

        # Validate data types
        column_types = {}
        if create_table_if_not_exists:
            column_types = await self.repo.create_table(table_name, df, pk_columns, embed_type, embed_columns_names)
        else:
        ...
        for col in df.columns:
            ...
            if pg_type in ('integer', 'bigint'):
                try:
                    df_converted[col] = pd.to_numeric(df_converted[col], errors='coerce').fillna(0).astype('int64')
                except Exception as e:
                    logger.warning(f"Failed to convert column {col} to numeric: {e}. Setting to 0.")
                    df_converted[col] = 0
            elif pg_type == 'date':
                df_converted[col] = pd.to_datetime(df_converted[col], errors='coerce')
            elif pg_type == 'text[]':
                df_converted[col] = df_converted[col].apply(lambda x: x if isinstance(x, list) else [])
        ...
        if embed_type == 'combined':
            df_converted['embed_columns_value'] = df_converted[embed_columns_names].apply(
                lambda row: self._format_embed_value(row, embed_columns_names), axis=1
            )
            embeddings = self.embedding_service.get_embeddings(df_converted['embed_columns_value'].tolist())
    ...
    async def _validate_schema(
        ...
        if not set(embed_columns_names).issubset(data_columns):
            raise DataValidationError(f"embed_columns_names {embed_columns_names} not in data columns {data_columns}")
        # Type mapping
        pd_to_pg = {
            'object': 'text',
        ...
        for col in data_columns:
            ...
            if df[col].apply(lambda x: isinstance(x, list)).any():
                ...
                if dtype == 'object' and len(non_null) > 0:
                    try:
                        if (non_null.apply(lambda x: isinstance(x, (str, float)) and str(x).replace('.', '').isdigit()).all() and
                            not non_null.apply(lambda x: isinstance(x, str) and len(x) > 20).any()):
                            expected_pg = 'bigint' if non_null.astype(float).max() > 2**31-1 else 'integer'
                        else:
                            expected_pg = 'text'
            ...
            if expected_pg != actual_pg:
                raise DataValidationError(f"Type mismatch for {col}: expected {expected_pg}, got {actual_pg}")
