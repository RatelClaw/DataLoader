...
from src.embdloader.domain.entities import TableSchema, DBOperationError, DataValidationError
...
class PostgresDataRepository(DataRepositoryInterface):
    ...
    async def create_table(self, table_name: str, df: pd.DataFrame, pk_columns: List[str], embed_type: str = 'combined', embed_columns_names: List[str] = []) -> Dict[str, str]:
        ...
        for col in pk_columns:
            if df[col].isnull().any():
                raise DataValidationError(f"Primary key column {col} contains null values")
                    ...
                    if dtype == 'object' and len(non_null) > 0:
                        try:
                            if (non_null.apply(lambda x: isinstance(x, (str, float)) and str(x).replace('.', '').isdigit()).all() and
                                ...
                                pg_type = 'bigint' if non_null.astype(float).max() > 2**31-1 else 'integer'
                            else:
                                pg_type = 'text'
    ...
    async def add_column(self, table_name: str, column_name: str, column_type: str):
        ...
        try:
            ...
            raise DBOperationError(f"Failed to add column {column_name} to {table_name}: {e}")
    ...
    async def insert_data(self, table_name: str, df: pd.DataFrame, pk_columns: List[str]):
        ...
        try:
            ...
            raise DBOperationError(f"Table {table_name} must be created before insertion")
        ...
        for col in columns:
            ...
            if pg_type in ('integer', 'bigint'):
                ...
                if nan_count > 0:
                    logger.warning(f"Column {col} had {nan_count} NaN values converted to 0")
            elif pg_type == 'date':
                df_converted[col] = pd.to_datetime(df_converted[col], errors='coerce')
                df_converted[col] = df_converted[col].apply(lambda x: x if pd.notnull(x) else None)
            elif pg_type == 'text[]':
                ...
                df_converted[col] = df_converted[col].apply(lambda x: str(x) if pd.notnull(x) else None)
            elif pg_type == f'vector({DEFAULT_DIMENTION})':
                df_converted[col] = df_converted[col].apply(lambda x: [float(v) for v in x] if x is not None else None)
    ...
    async def update_data(self, table_name: str, df: pd.DataFrame, pk_columns: List[str]):
        if df.empty:
            return
        columns = list(df.columns)
        set_clause = ', '.join([f"{col} = EXCLUDED.{col}" for col in columns if col not in pk_columns])
        query = f"""
    ...
    async def get_data_columns(self, table_name: str) -> List[str]:
        schema = await self.get_table_schema(table_name)
        return [col for col in schema.columns if col not in self.EXTRA_COLUMNS and not col.endswith('_enc')]